...
状態遷移表の正本は `claude-code/skills/status-table.md` にある。まず読む。**このスキルが動かすのは `Proposed`（scout-major が起票する）である。**
...
- `backlog task create`・`backlog milestone add` はタイトル・名前を引数で渡す（省略すると対話プロンプトになる）。
