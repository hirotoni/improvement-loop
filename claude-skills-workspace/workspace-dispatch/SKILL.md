...
複数の git リポジトリを直下（深さ1）にクローンした「ワークスペースディレクトリ」を対象に、[improvement-dispatch](../../claude-skills/improvement-dispatch/SKILL.md) の手順を opt-in 済みの各リポジトリへ順番に適用する薄いオーケストレーション層である。
